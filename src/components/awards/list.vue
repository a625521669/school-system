                        ...
                        <tr v-for="(item, index) in list">
                            ...
                            <td>
                                <div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
                                    <router-link :to="{name:'awardsedit', params:{id:item.ID}}" class="btn btn-xs btn-info">
...
<script>
    ...
    const _bindList = (self) => {
        fetch(self.$store.state.apiBase + '/awards/list?type=' + self.$route.params.type)
    ...
    export default {
        ...
        methods: {
            remove(id) {
                let self = this;
                if (confirm("确认删除吗?")) {
                    fetch(self.$store.state.apiBase + '/awards/delete?id=' + id)
                        .then(response => response.json())
                        .then(() => _bindList(self))
                }
                else {
